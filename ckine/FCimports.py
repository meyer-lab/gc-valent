...
def compMatrix(date, plate, panel, invert=True):
    ...
    # Add diagonal values of 100 to compensation values
    addedChannels = []
...
def subtract_unstained_signal(sample, channels, unstainedWell):
    ...
    meanBackground = np.zeros(len(channels))
    for i, channel in enumerate(channels):
        meanBackground[i] = np.mean(unstainedWell.data[channel])  # Calculates mean unstained signal
