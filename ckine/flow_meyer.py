...
def compMatrix(invert=True):
    ...
    df_comp = pd.read_csv(
        path, header=None, index_col=False, skiprows=0, names=header_names
    )
...
def make_flow_df(subtract=True):
        ...
        for i, row in enumerate(rows):
            sample = FCMeasurement(
                ...
                datafile=(
                    join(
                        path_here,
                        "ckine/data/Flow_Data_Meyer/Specimen_001_"
                        + column
                        + rowminus[i]
                        + "_"
                        + column
            ...
            for cell_type in cell_types:
                ...
                MeyerDF = pd.concat(
                    [
                        ...
                        pd.DataFrame(
                            {
                                "Ligand": lig_dict[row][0],
                                "Valency": lig_dict[row][1],
                                "Dose": dose_dict[column],
                                "Cell": cell_type,
                                "pSTAT5": [mean],
                                "Date": "6/25/22",
                            }
    ...
    MeyerDF = (
        MeyerDF.groupby(["Ligand", "Valency", "Dose", "Cell", "Date"])
        .pSTAT5.mean()
        .reset_index()
    )
    ...
    for cell_type in cell_types:
        ...
        untreatedDF = pd.concat(
            [untreatedDF, pd.DataFrame({"Cell": cell_type, "pSTAT5": [mean]})]
        )
        ...
        for valency in MeyerDF.loc[MeyerDF.Ligand == ligand].Valency.unique():
            for dose in MeyerDF.Dose.unique():
                for cell in MeyerDF.Cell.unique():
                    MeyerDF.loc[
                        (MeyerDF.Valency == valency)
                        & (MeyerDF.Ligand == ligand)
                        & (MeyerDF.Cell == cell)
                        & (MeyerDF.Dose == dose),
                        "pSTAT5",
                    ] -= untreatedDF.loc[untreatedDF.Cell == cell].pSTAT5.values
