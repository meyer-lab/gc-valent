                ...
                for j, row in enumerate(rows):
                    ...
                    if (row == 'H' and i == 4) is False:
                        ...
                        for ii, sampleii in enumerate(sample):  # get pstat data and put it into list form
                            ...
                            stat_array = stat_array.clip(min=1)  # remove small percentage of negative pstat values
                            if stat_array.size == 0:
                                MVdf = MVdf.append(pds.DataFrame.from_dict({"Date": dates[i], "Time": timeFunc(row), "Cell": TitlesNK[k],
                                                                            "Ligand": cytFunc(row), "Dose": dosemat[0, ii], "Mean": [0], "Variance": [0], "Skew": [0], "Kurtosis": [0], "alphStatCov": [0], "Bivalent": [0]}))
                            else:
                                MVdf = MVdf.append(pds.DataFrame.from_dict({"Date": dates[i], "Time": timeFunc(row), "Cell": TitlesNK[k], "Ligand": cytFunc(row), "Dose": dosemat[0, ii], "Mean": np.mean(stat_array), "Variance": np.var(
                                    stat_array), "Skew": stats.skew(stat_array), "Kurtosis": stats.kurtosis(stat_array), "alphStatCov": [0], "Bivalent": [0]}))
                    if j == 3 or j == 7:
                        MVdf['Mean'] = MVdf['Mean'] - MVdf['Mean'].min()
                        masterMVdf = masterMVdf.append(MVdf)
                        MVdf = pds.DataFrame(columns={"Date", "Time", "Ligand", "Dose", "Mean", "Variance", "Skew", "Kurtosis", "alphStatCov", "Bivalent"})
