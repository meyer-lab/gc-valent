...
def makeFigure():
    ...
    for i, ligand_name in enumerate(first_group_ligands):
        for j, cell_name in enumerate(dataMean.Cells.unique()):
            ...
            pred_data = calc_dose_response_mutein([1., 1., 5.], tps, muteinC, cell_receptors, exp_data) #TODO: Verify keeping 5x weaker endosomal assumption
            axis = i*8+j
            if axis == 15:
...
def calc_dose_response_mutein(input_params, tps, muteinC, cell_receptors, exp_data):
    ...
    for i, conc in enumerate(muteinC):
        active_ckine = runIL2simple(input_params, conc, tps=tps, input_receptors=cell_receptors, adj_receptors=True)
