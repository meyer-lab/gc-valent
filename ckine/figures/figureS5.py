...
def plot_exp_v_pred(ax, cell_subset=[]):
    """ Perform main routine for plotting all the cell predictions vs. experimental values. The default argument of cell_subset is an empty list which ends up plotting all 10 cell types; if one wishes to one plot a subset of cells they must be noted in list format. """
    ...
    axis = 0
    shift = 10 if len(cell_subset) == 0 else len(cell_subset)  # there are 10 cells if no subset is given

    for i, name in enumerate(cell_names_pstat):
        if name in cell_subset or len(cell_subset) == 0:  # if a subset is provided only plot the listed names
