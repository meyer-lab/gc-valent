...
def makeFigure():
    ...
    selectiveDF = pd.concat(
        [
            selectiveDF,
            pd.DataFrame({"Method": "Distance", "Marker": distDF.Marker.values}),
        ]
    )
    selectiveDF = pd.concat(
        [
            selectiveDF,
            pd.DataFrame({"Method": "RIDGE", "Marker": RIDGEdf.Marker.values}),
        ]
    )
    selectiveDF = pd.concat(
        [selectiveDF, pd.DataFrame({"Method": "SVM", "Marker": SVMdf})]
    )
