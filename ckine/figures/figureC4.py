...
def makeFigure():
    ...
    modelDF = runFullModel(
        time=[0.5, 1.0], saveDict=False, singleCell=True
    )  # Change to save
    ...
    ax[5].legend(
        legend.legendHandles, labels, loc="upper left", prop={"size": 8.5}
    ...
    IL2RaEffPlot(
        ...
        "Treg",
        IL2RBaff=1e8,
        IL2Ra_affs=np.array([1e8, 1e9, 1e10]),
        labels=["10", "1", "0.1"],
    )
    IL2RaEffPlot(
        ax[12],
...
def Pred_Exp_plot(ax, df):
    ...
    sns.scatterplot(
        x="Experimental", y="Predicted", style="Valency", data=df, ax=ax, alpha=0.35
    )
        ...
        for val in df.Valency.unique():
            ...
            accDF = pd.concat(
                [
                    accDF,
                    pd.DataFrame(
                        {"Cell Type": [cell], "Valency": [val], "Accuracy": [r2]}
                    ),
                ]
            )
...
def R2_Plot_Ligs(ax, df):
    ...
    for ligand in df.Ligand.unique():
        for val in df.loc[df.Ligand == ligand].Valency.unique():
            ...
            exps = df.loc[
                (df.Ligand == ligand) & (df.Valency == val)
            ].Experimental.values
            r2 = r2_score(exps, preds)
            if val == 1:
                accDF = pd.concat(
        ...
        for val in df.loc[(df.Dose == conc)].Valency.unique():
            ...
            accDF = pd.concat(
                [
                    accDF,
                    pd.DataFrame(
                        {"Concentration": [conc], "Valency": [val], "Accuracy": [r2]}
                    ),
                ]
            )
...
def MonVsBivalent(ax, dfAll, ligs=True):
    ...
    for date in dates:
        for cell in cellTypes:
            if cell[-1] == "$":  # if it is a binned pop, use ave fit
                predVecBin = df.loc[
                    (df.Date == date) & (df.Cell == cell)
                ].MonPredict.values
                expVec = dfCopy.loc[
                    (dfCopy.Date == date) & (dfCopy.Cell == cell[0:-13])
                ].Experimental.values
                predVec = dfCopy.loc[
                    (dfCopy.Date == date) & (dfCopy.Cell == cell[0:-13])
                ].MonPredict.values
                slope = np.linalg.lstsq(
                    np.reshape(predVec, (-1, 1)).astype(np.float),
                    np.reshape(expVec, (-1, 1)).astype(np.float),
                    rcond=None,
                )[0][0]
                df.loc[(df.Date == date) & (df.Cell == cell), "MonPredict"] = (
                    predVecBin * slope
                )
            else:
                expVec = df.loc[
    ...
    if ligs:
            ...
            accDF = pd.concat(
                [
                    ...
                    pd.DataFrame(
                        {
                            "Ligand": [ligand],
                            "Prediction Label": "Monovalent (Incorrect)",
                            "Accuracy": [r2Mon],
                        }
                    ),
                ]
            ...
            accDF = pd.concat(
                [
                    ...
                    pd.DataFrame(
                        {
                            "Ligand": [ligand],
                            "Prediction Label": "Bivalent (Correct)",
                            "Accuracy": [r2Biv],
                        }
                    ),
                ]
        ...
        sns.barplot(
            x="Cell Type", y="Accuracy", hue="Prediction Label", data=accDF, ax=ax
        )
        ax.set(ylim=(0, 1), ylabel=r"Accuracy ($R^2$)")
        ax.set_xticklabels(ax.get_xticklabels(), rotation=45)
...
def EC50comp(ax, dfAll, time):
    ...
    for ligand in Ligands:
        ...
        for valency in Valencies:
            for cell in Cells:
                ...
                dosesExp = df.loc[
                    (df.Ligand == ligand) & (df.Cell == cell) & (df.Valency == valency)
                ].Dose.values
                doseMax, doseMin = np.log10(np.amax(dosesExp)) + 4, np.log10(
                    np.amin(dosesExp)
                )
    ...
    sns.scatterplot(
        x="Ligand", y="EC50", hue="Cell Type", style="Exp/Pred", data=EC50df, ax=ax
    )
...
def timePlot(ax):
    ...
    for time in times:
        ...
        for val in df.Valency.unique():
            ...
            exps = df.loc[
                (df.Time == time[0]) & (df.Valency == val)
            ].Experimental.values
            r2 = r2_score(exps, preds)
            accDF = pd.concat(
                [
                    accDF,
                    pd.DataFrame({"Time": time, "Valency": [val], "Accuracy": [r2]}),
                ]
            )
...
def IL2RaEffPlot(ax, modelDF, cell, IL2RBaff, IL2Ra_affs, labels):
    ...
    concs = np.logspace(
        start=np.log10(modelDF.Dose.min()),
        stop=np.log10(modelDF.Dose.max()),
        num=101,
        endpoint=True,
    )
            ...
            for dose in concs:
                ...
                outputDF = pd.concat(
                    [
                        ...
                        pd.DataFrame(
                            {
                                r"IL2Rα $K_D$ (nM)": labels[i],
                                "Concentration": [dose],
                                "Valency": [val],
                                "Predicted pSTAT5 MFI": predVal,
                            }
                        ),
                    ]
                )
...
def recSigPlot(ax, modelDF, IL2RBrec, IL2Rarecs, IL2RBaff, IL2Ra_aff, label):
    ...
    for val in valencies:
        for alphaRec in IL2Rarecs:
            for dose in concs:
                recCount = np.ravel([alphaRec, IL2RBrec])
                predVal = polyc(
                    dose / 1e9 / val, getKxStar(), recCount, [[val, val]], [1.0], Affs
                )[0][1]
                outputDF = pd.concat(
                    [
                        outputDF,
    ...
    sns.lineplot(
        data=outputDF,
        x="Concentration",
        y="Active Binding Complexes",
        hue=r"IL2Rα Abundance",
        ax=ax,
        size="Valency",
    )
