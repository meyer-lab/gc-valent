        ...
        for j, _ in enumarate(cell_names_pstat):
            if cell_names_receptor[i] == cell_names_pstat[j]:
                ...
                if i == (receptor_data.shape[0] - 1):  # only plot the legend for the last entry
                    IL15_dose_response(ax[axis], unkVec_2_15, cell_names_receptor[i], receptor_data[i], ckineConc, legend=True)
                else:
                    IL15_dose_response(ax[axis], unkVec_2_15, cell_names_receptor[i], receptor_data[i], ckineConc)
                axis = axis + 1
