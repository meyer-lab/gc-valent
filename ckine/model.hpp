...
public:
	...
	explicit ratesS(const double * const rxntfR) {
        if (rxntfR[0] == 0.0) // first element is 0 for standard calculation (rxntfR size is Nparams+1)
        {
        ...
        else if (rxntfR[0] == 1.0) // case where IL-2 rates are inputs (rxntfR is of size 11)
        {
...
extern "C" int runCkine (double *tps, size_t ntps, double *out, const double * const rxnRatesIn, const bool sensi, double *sensiOut, const bool IL2_input);
