...
def import_pstat(combine_samples=True):
    ...
    dataMean = pds.DataFrame({'Cells': np.tile(np.repeat(cell_names, 48), 2), 'Ligand': np.concatenate((np.tile(np.array('IL2'), 480), np.tile(np.array('IL15'), 480))), 'Time':np.tile(np.repeat(tps, 12), 20), 'Concentration': np.tile(ckineConc, 80), 'RFU':np.concatenate((IL2_data.reshape(480,), IL15_data.reshape(480,)))})
