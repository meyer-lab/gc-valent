...
def polyc(L0, KxStar, Rtot, Cplx, Ctheta, Kav):
    ...
    lsq = root(
        ...
        Rtot,
        method="lm",
        args=(L0, KxStar, Rtot, Cplx, Ctheta, Kav),
...
def cytBindingModel(mut, val, doseVec, cellType, x=False, date=False):
    ...
    recCount = np.ravel(
        [
            recDF.loc[
                (recDF.Receptor == "IL2Ra") & (recDF["Cell Type"] == cellType)
            ].Mean.values,
            recDF.loc[
                (recDF.Receptor == "IL2Rb") & (recDF["Cell Type"] == cellType)
            ].Mean.values,
        ]
    )
    ...
    if date:
        ...
        if cellType[-1] == "$":  # if it is a binned pop, use ave fit
            output *= convDict.loc[
                (convDict.Date == date) & (convDict.Cell == cellType[0:-13])
            ].Scale.values
        else:
            output *= convDict.loc[
                (convDict.Date == date) & (convDict.Cell == cellType)
            ].Scale.values
    return output
...
def runFullModel(x=False, time=[0.5], saveDict=False, singleCell=False):
    ...
    for (date, lig, conc, cell, time), group in statDF.groupby(
        ["Date", "Ligand", "Dose", "Cell", "Time"]
    ):
        if lig[-5::] == "(Biv)":
            val = 2
            ligName = lig[0:-6]
    ...
    for date in dates:
        for cell in masterSTAT.Cell.unique():
            if cell[-1] == "$":  # if it is a binned pop, use ave fit
                predVecBin = masterSTAT.loc[
                    (masterSTAT.Date == date) & (masterSTAT.Cell == cell)
                ].Predicted.values
                slope = dateConvDF.loc[
                    (dateConvDF.Date == date) & (dateConvDF.Cell == cell[0:-13])
                ].Scale.values
                masterSTAT.loc[
                    (masterSTAT.Date == date) & (masterSTAT.Cell == cell), "Predicted"
                ] = (predVecBin * slope)
            else:
                expVec = masterSTAT.loc[
                    (masterSTAT.Date == date) & (masterSTAT.Cell == cell)
                ].Experimental.values
                predVec = masterSTAT.loc[
                    (masterSTAT.Date == date) & (masterSTAT.Cell == cell)
                ].Predicted.values
                slope = np.linalg.lstsq(
                    np.reshape(predVec.astype(np.float), (-1, 1)),
                    np.reshape(expVec.astype(np.float), (-1, 1)),
                    rcond=None,
                )[0][0]
                masterSTAT.loc[
                    (masterSTAT.Date == date) & (masterSTAT.Cell == cell), "Predicted"
                ] = (predVec * slope)
                dateConvDF = pd.concat(
                    [
                        dateConvDF,
                        pd.DataFrame({"Date": date, "Scale": slope, "Cell": cell}),
                    ]
                )
    if saveDict:
        dateConvDF.set_index("Date").to_csv(
            join(path_here, "ckine/data/BindingConvDict.csv")
        )

    if x:
        return np.linalg.norm(
            masterSTAT.Predicted.values - masterSTAT.Experimental.values
        )
    else:
        return masterSTAT
...
def runFullModelMeyer(x=False, saveDict=False):
    ...
    for (date, lig, val, conc, cell), group in statDF.groupby(
        ["Date", "Ligand", "Valency", "Dose", "Cell"]
    ):
    ...
    for date in dates:
        for cell in masterSTAT.Cell.unique():
            ...
            dateConvDF = pd.concat(
                [dateConvDF, pd.DataFrame({"Date": date, "Scale": slope, "Cell": cell})]
            )
    if saveDict:
        dateConvDF.set_index("Date").to_csv(
            join(path_here, "ckine/data/BindingConvDictMeyer.csv")
        )
